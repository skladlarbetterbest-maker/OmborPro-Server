...
const Reports = {
  ...
  populateObyekt() {
    ...
    // Omborlarni arrayga aylantirish (agar string bo'lsa)
    ...
    omborlar = Array.from(new Set(omborlar)); // Dublikatlarni olib tashlash

    let allowed = App.currentUser?.ombor || 'Barchasi';
    // Allowed omborni ham arrayga aylantirish
    ...
    if (!isAdmin && !allowed.includes('Barchasi')) {
      opts = omborlar.filter(o => allowed.includes(o)).map(o => `<option value="${o}">${o}</option>`).join('');
    } else {
      opts = '<option value="Barchasi">Barchasi</option>' + omborlar.filter(o=>o!=='Barchasi').map(o => `<option value="${o}">${o}</option>`).join('');
    }
    el.innerHTML = opts;
  },
  async generate() {
