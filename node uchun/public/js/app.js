...
const App = {
  ...
  populateObyektFilters() {
    ...
    // Omborlarni barcha joylarda ishlatish (Kirim, Chiqim, Transfer, Inventarizatsiya)
    const omborlar = this.data.omborlar || [];
    const omborOpts = omborlar.map(o => `<option value="${o}">${o}</option>`).join('');
    
    ['qoldiq-obyekt-filter', 'jurnal-obyekt-filter'].forEach(id => {
      const el = document.getElementById(id);
      if (el) {
        const filterOpts = '<option value="Barchasi">Barchasi</option>' + omborOpts;
    ...
    ['k-obyekt', 'c-oluvchi', 'trf-from', 'inv-obyekt'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = omborOpts;
    });

    // trf-to hamma omborlarni o'z ichiga oladi
    const trfToEl = document.getElementById('trf-to');
    if (trfToEl) trfToEl.innerHTML = omborOpts;
