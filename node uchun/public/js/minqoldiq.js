...
const MinQoldiq = {
  ...
  render() {
    ...
    const ombor = document.getElementById('minq-obyekt-filter')?.value || 'Barchasi';
    ...
    // Filter by ombor
    let filtered = jurnal.filter(r => ombor === 'Barchasi' || (r.obyekt || 'Barchasi') === ombor);
  ...
  populateObyektFilter() {
    ...
    if (!select) return;
    let omborlar = App.data.omborlar || ['Barchasi'];
    const user = App.currentUser;
    let userOmbor = user?.ombor || 'Barchasi';

    // Omborlarni arrayga aylantirish
    if (typeof omborlar === 'string') {
      omborlar = omborlar.split(',').map(o => o.trim());
    }
    if (Array.isArray(omborlar)) {
      omborlar = omborlar.flatMap(o => {
    ...
    omborlar = Array.from(new Set(omborlar));

    // User omborni ham arrayga aylantirish
    if (typeof userOmbor === 'string') {
      if (userOmbor.includes(',')) {
        userOmbor = userOmbor.split(',').map(o => o.trim());
      } else {
        userOmbor = [userOmbor];
      }
    }
    if (Array.isArray(userOmbor)) {
      userOmbor = userOmbor.flatMap(o => {
        if (typeof o === 'string' && o.includes(',')) {
          return o.split(',').map(x => x.trim());
        }
        return o;
    ...
    userOmbor = Array.from(new Set(userOmbor));

    select.innerHTML = omborlar
      .filter(o => userOmbor.includes('Barchasi') || userOmbor.includes(o) || o === 'Barchasi')
